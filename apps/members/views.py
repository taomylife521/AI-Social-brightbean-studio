...
def member_list(request):
    ...
    memberships = (
        OrgMembership.objects.filter(organization=org)
        .select_related("user")
        .order_by("invited_at")
    )
...
def invite_member(request):
    ...
    if request.headers.get("HX-Request"):
        return render(request, "members/partials/invitation_row.html", {
            "invite": invitation,
            "is_admin": True,
        })
...
def resend_invite(request, invitation_id):
    ...
    invitation = get_object_or_404(
        Invitation, id=invitation_id, organization=request.org
    )
    try:
...
def accept_invite(request, token):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            return render(request, "members/accept_invite.html", {
...
def update_member_role(request, membership_id):
    ...
    membership = get_object_or_404(
        OrgMembership, id=membership_id, organization=request.org
    )
    ...
    if request.headers.get("HX-Request"):
        ...
        org_workspace_ids = Workspace.objects.filter(
            organization=request.org, is_archived=False
        ...
        return render(request, "members/partials/member_row.html", {
            "member": {
...
def manage_workspaces(request, membership_id):
    ...
    return render(request, "members/partials/workspace_assignments.html", {
        "membership": membership,
